...
# 3. Input Gamma Energies
 ...
 For each energy index 'i', the simulation produces a output<i>.root stored in 'results' folder
...
# How to run the simulation
 first make a build directory, then inside build directory run the follow linux command cmake .. && make -j4 && ./sim run.mac, after running these commands 
 all the output*.root files will be generated then we have to run the root macro named "efficiency_analysis.C" which is present in the macros folder 
 ( it is important to run the macro inside build folder as all the root output files are present in build), running the root macro will give the output plots.
